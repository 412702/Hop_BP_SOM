...
class Util{
    ...
    static get_random_int(max) {
        return Math.floor(Math.random() * Math.floor(max));
    }

    static to_1d(_input_2d) {
   ...
   static get_random_int_min_max(min, max) {
        return Math.random() * (max - min) + min;
    }

    static to_2d(_input_1d) {
    ...
    static pixel_to_index(x, y, dim, width) {
        console.log(x, y)
        let idim = width / dim;
        return [Math.floor(x / idim), Math.floor(y / idim)]
    }

    static generate_random_bipolar_vector(_num_unit) {
        let _bipolar_1d = []
        for (var i = 0; i < _num_unit; ++i) {
            let rand_num = this.get_random_int_min_max(-10, 10);
            _bipolar_1d.push(Math.sign(rand_num));
        }
        return _bipolar_1d
    }

    static add_noise_to_vector(_input_vector, percentage) {
        let max_value = Math.round(percentage * _input_vector.length);
        for (var i = 0; i < max_value; ++i) {
            let rand_int = this.get_random_int(_input_vector.length - 1);
