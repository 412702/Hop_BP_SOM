...
function FCNN(selector) {
    ...
    var svg = d3.select(selector).append("svg").attr("xmlns", "http://www.w3.org/2000/svg");

    // var svg = d3.select('#bp-graph-container').append("svg").attr("xmlns", "http://www.w3.org/2000/svg");
    var g = svg.append("g");
...
//  不想搞源码了，就在这里直接改了，只需要显示就行了，不做太多可操作性的内容
    // svg.call(d3.zoom()
    //            .scaleExtent([1 / 2, 8])
    //            .on("zoom", zoomed));
    ...
    function resize() {
        // w = d3.select(selector)._groups[0][0].parentElement.offsetWidth;
        // h = 400;
        w = window.innerWidth;
